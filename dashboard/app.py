...
class FinancialInclusionDashboard:
    ...
    def render_overview(self):
        ...
        st.markdown('<div class="sub-header">💱 P2P Digital Transfers vs ATM Withdrawals</div>', unsafe_allow_html=True)
    ...
    def render_forecasts(self):
        ...
        st.markdown('<h1 class="main-header">🔮 Inclusion Forecasts 2025-2027</h1>', unsafe_allow_html=True)
        ...
        if forecasts:
            ...
            with col1:
                acc_forecast = forecasts['ACC_OWNERSHIP']['scenarios'][2027][scenario]['value']
            ...
            with col2:
                pay_forecast = forecasts['USG_DIGITAL_PAYMENT']['scenarios'][2027][scenario]['value']
            ...
            with col3:
                mm_forecast = forecasts['ACC_MM_ACCOUNT']['scenarios'][2027][scenario]['value']
            ...
            st.markdown('<div class="sub-header">📈 Forecast Trajectories</div>', unsafe_allow_html=True)
            ...
            colors = {'ACC_OWNERSHIP': 'blue', 'USG_DIGITAL_PAYMENT': 'green', 'ACC_MM_ACCOUNT': 'orange'}
            ...
            for indicator_code, color in colors.items():
                if indicator_code in forecasts:
                    ...
                    for year in [2025, 2026, 2027]:
                        values.append(forecasts[indicator_code]['scenarios'][year][scenario]['value'])
            ...
            st.markdown('<div class="sub-header">🎯 Target Achievement Assessment</div>', unsafe_allow_html=True)
            
            achievement_data = []
            for indicator_code in ['ACC_OWNERSHIP', 'USG_DIGITAL_PAYMENT', 'ACC_MM_ACCOUNT']:
                if indicator_code in forecasts:
                    forecast_2027 = forecasts[indicator_code]['scenarios'][2027][scenario]['value']
            ...
            st.markdown('<div class="sub-header">📊 Forecast Uncertainty</div>', unsafe_allow_html=True)
            
            uncertainty_data = []
            for indicator_code in ['ACC_OWNERSHIP', 'USG_DIGITAL_PAYMENT']:
                if indicator_code in forecasts:
                    optimistic = forecasts[indicator_code]['scenarios'][2027]['optimistic']['value']
                    base = forecasts[indicator_code]['scenarios'][2027]['base']['value']
                    pessimistic = forecasts[indicator_code]['scenarios'][2027]['pessimistic']['value']
    ...
    def render_policy_simulator(self):
        ...
        st.markdown('<h1 class="main-header">🎮 Policy Impact Simulator</h1>', unsafe_allow_html=True)
        ...
        # Calculate impacts
        st.markdown('<div class="sub-header">📊 Simulated Impacts</div>', unsafe_allow_html=True)
        ...
        impacts = {
            'Digital ID': digital_id * 0.3,  # 30% of coverage increase translates to account increase
            'KYC Simplification': {
                'No Change': 0,
                'Moderate': 2,
                'Significant': 4
            ...
            'Network Coverage': (network_coverage - 70) * 0.15,  # 15% of coverage increase
            'Interoperability': {
                'Basic': 1,
                'Enhanced': 3,
                'Full': 5
        ...
        if digital_id < 20:
            recommendations.append("Consider accelerating digital ID rollout to reach more underserved populations")
        
        if kyc_simplification != 'Significant':
            recommendations.append("Further KYC simplification could significantly boost account opening")
        
        if agent_expansion < 30:
            recommendations.append("Agent network expansion is crucial for rural inclusion")
        
        if len(recommendations) > 0:
            for rec in recommendations:
                st.info(f"• {rec}")
        else:
            st.success("Your policy mix appears well-balanced for achieving inclusion targets!")
