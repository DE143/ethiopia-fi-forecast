...
class DataLoader:
        ...
        events = pd.DataFrame(
            {
                ...
                "event_date": pd.to_datetime(
                    ["2021-05-01", "2023-08-01", "2022-07-01"]
                ),
            }
        )
    ...
    def validate_data(self):
        ...
        invalid_record_types = set(self.raw_data["record_type"]) - set(
            valid_record_types
        )
        validation_results["record_type"] = len(invalid_record_types) == 0

        # Check pillar for observations
        valid_pillars = self.reference_codes[self.reference_codes["field"] == "pillar"][
            "code"
        ].tolist()
