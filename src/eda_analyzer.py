...
class EDAAnalyzer:
    def __init__(self, data_path='data/processed/enriched_data.csv'):
        self.data = pd.read_csv(data_path, parse_dates=['observation_date', 'event_date', 'created_at'])
    ...
    def analyze_access_trends(self):
        ...
        access_data['years_between'] = access_data['period'] - access_data['prev_period']
        access_data['annual_growth'] = access_data['growth_pp'] / access_data['years_between']
        
        insights = {
            'current_rate': access_data['value_numeric'].iloc[-1] if len(access_data) > 0 else None,
            'latest_growth': access_data['growth_pp'].iloc[-1] if len(access_data) > 1 else None,
            'avg_annual_growth': access_data['annual_growth'].mean() if len(access_data) > 1 else None,
            'slowdown_2024': access_data['growth_pp'].iloc[-1] < access_data['growth_pp'].iloc[-2] 
            if len(access_data) > 2 else None,
            'trajectory': access_data[['observation_date', 'value_numeric']].to_dict('records')
        }
    ...
    def analyze_usage_trends(self):
        ...
        payment_data = self.observations[
            (self.observations['indicator_code'].isin(['USG_DIGITAL_PAYMENT', 'ACC_MM_ACCOUNT'])) &
        ...
        if len(mm_data) > 0:
            ...
            mm_data['active_estimate'] = mm_data['value_numeric'] * 0.5  # 50% activity rate
            
        insights = {
            'mobile_money_growth': mm_data['value_numeric'].pct_change().mean() * 100 
            if len(mm_data) > 1 else None,
            'registered_active_gap': (mm_data['value_numeric'].iloc[-1] - mm_data['active_estimate'].iloc[-1]) 
            if len(mm_data) > 0 else None,
            'payment_trend': payment_data.groupby('indicator_code')['value_numeric'].last().to_dict()
        }
    ...
    def analyze_infrastructure_correlations(self):
        ...
        insights = {
            ...
            ]['value_numeric'].pct_change().mean() * 100 
            if len(infra_data[infra_data['indicator_code'] == 'AGENT_DENSITY']) > 1 else None,
            'coverage_growth': infra_data[
                infra_data['indicator_code'] == '4G_COVERAGE'
            ]['value_numeric'].pct_change().mean() * 100 
            if len(infra_data[infra_data['indicator_code'] == '4G_COVERAGE']) > 1 else None
        }
    ...
    def create_event_timeline(self):
        ...
        indicator_data = self.observations[
            self.observations['indicator_code'].isin(['ACC_OWNERSHIP', 'ACC_MM_ACCOUNT'])
        ].copy()
        indicator_data['year'] = indicator_data['observation_date'].dt.year
        
        timeline_data = {
            'events': key_events[['name', 'event_date', 'category']].to_dict('records'),
            'indicators': indicator_data.groupby(['indicator_code', 'year'])['value_numeric'].last().unstack().to_dict(),
            'event_impacts': self.impact_links[['parent_id', 'related_indicator', 'impact_magnitude']].to_dict('records')
        }
    ...
    def analyze_gender_gap(self):
        ...
        insights = {
            ...
            'gap_trend': 'stable' if gender_data['gender_gap'].std() < 2 else 'changing',
            'female_growth': gender_data['female_ownership'].pct_change().mean() * 100,
            'closing_rate': (gender_data['gender_gap'].iloc[-1] - gender_data['gender_gap'].iloc[0]) / 
    ...
    def analyze_urban_rural_gap(self):
        ...
        insights = {
            ...
            'rural_growth': urban_rural_data['rural_ownership'].pct_change().mean() * 100,
            'gap_trend': 'narrowing' if urban_rural_data['urban_rural_gap'].diff().iloc[-1] < 0 else 'widening'
        }
    ...
    def create_visualizations(self, output_dir='reports/figures'):
        ...
        ax1.plot(access_data['observation_date'], access_data['value_numeric'], 
        ...
        if len(mm_data) > 0:
            ...
            ax3.plot(mm_data['observation_date'], mm_data['value_numeric']/1000000, 
        ...
        for idx, event in events.iterrows():
            ...
            ax4.text(event['event_date'], 0.5, event['name'][:20], 
        ...
        ax5.bar(x - width/2, gender_data['male_ownership'], width, label='Male', alpha=0.8)
        ax5.bar(x + width/2, gender_data['female_ownership'], width, label='Female', alpha=0.8)
        ax5.set_xlabel('Year')
        ...
        return {
            ...
            'mobile_money_growth': f'{output_dir}/mobile_money_growth.png' if len(mm_data) > 0 else None,
