...
class EventImpactModeler:
    ...
    def estimate_event_impacts(self, indicator, event_date):
        ...
        for _, impact in relevant_impacts.iterrows():
            ...
            effective_date = impact['event_date'] + pd.DateOffset(months=impact['lag_months'])
    ...
    def validate_impacts_historically(self):
        ...
        for event_name, event_info in key_events.items():
            for indicator in event_info['affected_indicators']:
                ...
                pre_value = self.get_indicator_value(indicator, event_info['pre_period'])
                post_value = self.get_indicator_value(indicator, event_info['post_period'])
                ...
                if pre_value is not None and post_value is not None:
                    ...
                    validation_results.append({
                        ...
                        'accuracy': 1 - abs(modeled_impact - actual_change) / max(abs(actual_change), 0.1),
    ...
    def scenario_analysis(self, future_events):
        ...
        for scenario_name, events in future_events.items():
            ...
            scenarios.append({
                ...
                'events': events,
                'total_impact': sum(imp['estimated_impact'] for imp in scenario_impacts),
    ...
    def estimate_base_impact(self, event):
        ...
        comparable = comparable_events.get(event_type, comparable_events['new_mobile_money_operator'])
    ...
    def create_visualizations(self, output_dir='reports/figures'):
        ...
        if not validation_df.empty:
            ...
            ax.bar(x - width/2, validation_df['actual_change'], width, label='Actual Change', alpha=0.8)
            ax.bar(x + width/2, validation_df['modeled_impact'], width, label='Modeled Impact', alpha=0.8)
            ...
            ax.set_xticks(x)
            ax.set_xticklabels(validation_df.apply(
        ...
        ax.fill_between(timeline_df['date'], 0, timeline_df['cumulative_impact'], alpha=0.3)
        ax.set_xlabel('Date')
        ...
        return {
            ...
            'model_validation': f'{output_dir}/model_validation.png' if not validation_df.empty else None,
