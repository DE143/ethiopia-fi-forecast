...
class FinancialInclusionForecaster:
    ...
    def prepare_forecast_data(self, indicator_code='ACC_OWNERSHIP'):
        ...
        annual_data = indicator_data.groupby('year')['value_numeric'].mean().reset_index()
    ...
    def forecast_trend_model(self, indicator_code, forecast_years=[2025, 2026, 2027]):
        ...
        poly_predictions = lr_poly.predict(poly.transform(np.array(forecast_years).reshape(-1, 1)))
        ...
        try:
            ...
            if sum(valid_idx) >= 3:
                ...
                log_predictions_scaled = lr_log.predict(np.array(forecast_years).reshape(-1, 1))
    ...
    def forecast_event_augmented(self, indicator_code, forecast_years=[2025, 2026, 2027]):
        ...
        for year in forecast_years:
            ...
            impact, _ = event_modeler.estimate_event_impacts(indicator_code, target_date)
    ...
    def generate_scenarios(self, indicator_code, forecast_years=[2025, 2026, 2027]):
        ...
        augmented_results = self.forecast_event_augmented(indicator_code, forecast_years)
        ...
        base_model = 'polynomial' if 'polynomial' in augmented_forecasts else list(augmented_forecasts.keys())[0]
            ...
            if len(predictions) >= 2:
                ...
                if len(historical_data) >= 3:
                    ...
                    uncertainty_results[model_name] = {
                        'mean': mean_pred,
                        'std': std_pred,
                        'confidence_interval': (mean_pred - margin_error, mean_pred + margin_error),
    ...
    def create_forecast_visualizations(self, forecasts, output_dir='reports/figures'):
        ...
        if 'ACC_OWNERSHIP' in forecasts:
            ...
            for scenario, color in [('optimistic', 'green'), ('base', 'blue'), ('pessimistic', 'red')]:
                values = [acc_forecast['info']['current_value']]
                for year in [2025, 2026, 2027]:
                    values.append(acc_forecast['scenarios'][year][scenario]['value'])
                
                ax.plot(years, values, '--', color=color, linewidth=2, label=f'{scenario.capitalize()}')
                ax.fill_between(years, values, alpha=0.1, color=color)
            
            # Target line
            ax.axhline(y=60, color='black', linestyle=':', linewidth=1, label='NFIS-II Target (60%)')
            ...
            viz_files['account_forecast'] = f'{output_dir}/account_ownership_forecast.png'
        ...
        for idx, indicator_code in enumerate(indicators_to_plot):
            if indicator_code in forecasts and 'scenarios' in forecasts[indicator_code]:
                ...
                for scenario, color in [('optimistic', 'green'), ('base', 'blue'), ('pessimistic', 'red')]:
                    ...
                    ax.plot(years, values, '--', color=color, linewidth=2, label=scenario.capitalize())
        ...
        for indicator_code in indicators_to_plot:
            if indicator_code in forecasts and 'target_gap_2027' in forecasts[indicator_code]:
                indicator_name = forecasts[indicator_code]['info']['name']
                base_pred = forecasts[indicator_code]['target_gap_2027']['base']['prediction']
